...
import { XIcon } from "@/components/icons/flora";
...
>(({ className, children, ...props }, ref) => {
  // Recursively check if DialogDescription exists in children
  const hasDescription = React.useMemo(() => {
    const checkForDescription = (node: React.ReactNode): boolean => {
      return React.Children.toArray(node).some((child) => {
        if (!React.isValidElement(child)) return false;
        
        const childType = child.type as any;
        
        // Check if this is DialogDescription
        if (
    ...
    return checkForDescription(children);
  }, [children]);
